...
def init_parallel_jobs(dbs, queue, fn, data_aug):
    """创建prefetch_data的各进程"""
    tasks = [Process(target=prefetch_data, args=(db, queue, fn, data_aug)) for
             db in dbs]
    for task in tasks:
        task.daemon = True
        task.start()
...
def train(training_dbs, validation_db, start_iter=0):
    ...
    training_tasks = init_parallel_jobs(training_dbs, training_queue,
                                        sample_data, True)
    if val_iter:
        validation_tasks = init_parallel_jobs([validation_db], validation_queue,
                                              sample_data, False)
    ...
    # or use pre-trained model to do warm start
    # if pretrained_model is not None:
    #     if not os.path.exists(pretrained_model):
    #         raise ValueError("pretrained model does not exist")
    #     print("loading from pretrained model")
    #     nnet.load_warmstart_params(pretrained_model)
    ...
    with stdout_to_tqdm() as save_stdout:
        for iteration in tqdm(range(start_iter + 1, max_iteration + 1),
            ...
            if display and iteration % display == 0:
                print("training loss at iteration {}: {}".format(iteration,
                                                                 training_loss.item()))
                print("focal loss at iteration {}:    {}".format(iteration,
                                                                 focal_loss.item()))
                print("pull loss at iteration {}:     {}".format(iteration,
                                                                 pull_loss.item()))
                print("push loss at iteration {}:     {}".format(iteration,
                                                                 push_loss.item()))
                print("regr loss at iteration {}:     {}".format(iteration,
                                                                 regr_loss.item()))
            ...
            # Currently, we do not validate here
...
if __name__ == "__main__":
    ...
    training_dbs = [datasets[dataset](configs["db"]) for _ in range(threads)]
    validation_db = datasets[dataset](configs["db"])
